            ...
            if grid.get(i, j) == Some('X') {
                for u in -1 ..=1 {
                    for v in -1 ..=1 {
                        ...
                        match (grid.get(i+u, j+v), grid.get(i+2*u, j+2*v), grid.get(i+3*u, j+3*v)) {
                            (Some('M'), Some('A'), Some('S')) => results += 1,
                            _ => (),
                        }
                    }
                }
    ...
    for i in 0.. grid.rows() {
        for j in 0 .. grid.columns() {
            ...
            if grid.get(i, j) == Some('A') {
                ...
                match (grid.get(i-1, j-1), grid.get(i+1, j+1), grid.get(i+1, j-1), grid.get(i-1, j+1)) {
...
fn parse(input: &str) -> (Vec<String>, Vec<String>) {
    let mut columns: Vec<String> = Vec::with_capacity(150);
    for _ in 0.. 150 {
        columns.push(String::with_capacity(150));
    }
